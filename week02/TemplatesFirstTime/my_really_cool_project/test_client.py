...
class KeyValueStoreApiTests(unittest.TestCase):
    def test_create_user_is_successful(self):
        ...
        self.assertTrue(is_uuid4(data['identifier']), 'identifier should be a validentifier uuid')
    ...
    def test_cannot_get_other_user_key(self):
        ...
        identifier1 = r1.json()['identifier']
        identifier2 = r2.json()['identifier']

        store_key(identifier1, key='foo', value='bar')
        response = get_key(identifier2, key='foo')
...
class KeyValueHtmlViewsTests(unittest.TestCase):
    ...
    def test_index_view_contains_desired_elements(self):
        identifier1 = create_user().json()['identifier']
        identifier2 = create_user().json()['identifier']

        response = requests.get(BASE_URL + INDEX_VIEW)
        content = response.content.decode('utf-8')

        self.assertIn('<table>', content, 'There should be at least 1 table')
        self.assertIn(identifier1, content, 'First user should be there')
        self.assertIn(identifier2, content, 'Second user should be there')
        self.assertIn(USER_DETAIL_VIEW.format(identifier=identifier1), content, 'There should be a link for user1')
        self.assertIn(USER_DETAIL_VIEW.format(identifier=identifier2), content, 'There should be a link for user2')
